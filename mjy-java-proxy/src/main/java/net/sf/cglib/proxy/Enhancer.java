...
public class Enhancer extends AbstractClassGenerator
{
    ...
    private Class[] interfaces; //代理类需要实现的接口
    private CallbackFilter filter; // 回调方法过滤器
    private Callback[] callbacks; // 回调方法
    private Type[] callbackTypes; // 回调方法类，一般不用
    private boolean validateCallbackTypes;
    private boolean classOnly;
    private Class superclass;// 可以指定父类
     ...
     * 当使用createClass方法生成代理类的类的时候，可以用来替代setCallback。毕竟使用类不需要实例化，因此也不需要实例化的回调方法。
